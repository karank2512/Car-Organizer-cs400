...
public class BinarySearchTree<T extends Comparable<T>> implements SortedCollectionInterface<T> {
    ...
    protected void rotate(Node<T> child, Node<T> parent) throws IllegalArgumentException {
        ...
        // Start by checking exception to check if the parent and child nodes currently in considerations are related or not

        if((!child.equals(parent.down[0]) && !child.equals(parent.down[1]) && !child.equals(parent))|| child == null || parent == null){
            throw new IllegalArgumentException();
        }

        // checking if the child is the left child
        if(parent.down[0] == child){

            // because the child is the left child, we perform right rotation 

            Node<T> newNode = parent;
            newNode.down[0] = child.down[1];
            if(child.down[1]!= null){
                child.down[1].up = newNode;
            }
            child.down[1] = newNode;

        } 

        // checking if the child is the right child

        else if(parent.down[1] == child){

            // because the child is the right child, we perform left rotation 

            Node<T> newNode = parent;
            newNode.down[1] = child.down[0];
            if(child.down[0]!= null){
                child.down[0].up = newNode;
            }
            child.down[0] = newNode;
        }

        // after performing all the rotations, we update the parent references

        child.up = parent.up;

        // if the parent is the root, then update root to become child

        if (parent.up == null) {
            root = child;
        } else {
            if (parent.up.down[0] == parent) parent.up.down[0] = child;
            else parent.up.down[1] = child;
        }
        parent.up = child;
    }
    ...
    /* This method tests the rotate method for the IllegalArgumentException case. We tried to rotate two nodes which aren't related */
    public static boolean test1() {
        ...
        // find nodes for 4, and 5
        Node<Integer> four = tree.root;
        Node<Integer> five = four.down[1].down[0];

        // then rotate 5 and 4
        try {
            tree.rotate(five, four);
            return false;
        }
    ...
    public static boolean test2() {
        ...
        // then rotate 6 and 4
            ...
            if (!tree.toLevelOrderString().equals("[ 6, 4, 7, 2, 5, 1, 3 ]")) {
                ...
                System.out.println("After inserting 4, 2, 1, 3, 6, 5, 7 into an empty tree and " +
                        "rotating 7 and 5, level order is not [ 6, 4, 7, 2, 5, 1, 3  ], but should be");
                return false;
            }
    ...
    public static boolean test3() {
        ...
        // then rotate 5 and 6
            ...
            if (!tree.toLevelOrderString().equals("[ 2, 1, 4, 3, 6, 5, 7 ]")) {
                System.out.println("After inserting 4, 2, 1, 3, 6, 5, 7 into an empty tree and " +
                        "rotating 5 and 6, level order is not [ 2, 1, 4, 3, 6, 5, 7 ], but should be");
                return false;
            }
